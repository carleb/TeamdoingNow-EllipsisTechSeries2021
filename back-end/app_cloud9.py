...
def stratTest(stock, fractalIndicator, maIndicator, maPeriod, maSource, maOffset, capital, tpdetails, sldetails, ordersize):
    ...
    client = pymongo.MongoClient('mongodb://doingnow:master123@docdb-2021-08-28-14-47-24.cpa303rjn58z.us-west-2.docdb.amazonaws.com:27017/?replicaSet=rs0&readPreference=secondaryPreferred&retryWrites=false')
    ##Specify the database to be used
    db = client.history_ticker
    ##Specify the collection to be used
    col = db.ticker_data
    stock = stock.upper()
    # df = get_data(stock, start_date= "25/08/2020", end_date= "25/08/2021", index_as_date= False, interval="1d")
    start_date = int(str(int(datetime.strptime('25/08/2020', '%d/%m/%Y').timestamp())) + '000')
    end_date = int(str(int(datetime.strptime('25/08/2021', '%d/%m/%Y').timestamp())) + '000')
    
    query_data = col.find( {'ticker': stock, 'date': {'$lt': end_date, '$gte': start_date}})
...
def getPriceGraph(stock_symbol, indicators):
    ...
    start_date = int(str(int(datetime.strptime('25/08/2020', '%d/%m/%Y').timestamp())) + '000')
    end_date = int(str(int(datetime.strptime('25/08/2021', '%d/%m/%Y').timestamp())) + '000')
    query_data = col.find( {'ticker': stock_symbol, 'date': {'$lt': end_date, '$gte': start_date}})
